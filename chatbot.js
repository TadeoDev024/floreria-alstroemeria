...
chatFormWidget.addEventListener('submit', async (e) => {
  ...
  // Mostrar "Escribiendo..."
  const loadingId = addMessageWidget('Thinking...', 'bot', true);
  ...
  try {
    ...
    if (data.error) addMessageWidget('Lo siento, hubo un error de conexión.', 'bot');
    else addMessageWidget(data.texto, 'bot');

  } catch (err) {
    removeMessage(loadingId);
    addMessageWidget('Sin conexión con el servidor.', 'bot');
  }
});
